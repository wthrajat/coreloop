...
QStash wakes the application every ten minutes and dispatches the oldest due
job. A finished worker then wakes exactly one next due job, so the backlog
drains chronologically without waiting ten minutes between every item. Turso
remains the queue's source of truth, so a later tick safely resumes a broken
handoff. This serial dispatch deliberately avoids a burst of concurrent workers
against the Turso Free database.
...
Coreloop validates the numeric OIDC `id` with Telegram `getChat` during this
callback. If an account was created by a revision that stored `sub` as the chat
ID, sign out and complete Telegram Login once after deploying the corrected
...
- In Settings, keep Radar enabled, set the daily target to 8, and enable its
  independent weekend option. Save, reload, and confirm all three values remain.
- Run signed ticks until source ingestion and ranking have completed. Confirm a
  Radar briefing arrives outside lesson delivery, uses simple neutral language,
  names the original source, and includes a working **Open source** button.
...
- Temporarily replace both free keys with invalid non-empty values, redeploy,
  and confirm a lesson job becomes quota blocked, remains in Turso, and the user
  sees the quota message. Production validation requires at least one key to be
  present, so do not remove both variables.
- During the same provider-failure window, confirm a source-backed Radar item
  still reaches Telegram through the deterministic fallback.
...
2. update `APP_ORIGIN` in Vercel and locally;
...
- Sign out and complete Telegram Login again so Coreloop refreshes and validates
  the Bot API chat ID.
