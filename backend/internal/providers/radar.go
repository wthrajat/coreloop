...
func (router *Router) EnrichRadar(ctx context.Context, input RadarInput) (RadarEnrichment, error) {
	...
	system := "Rewrite one technology-news item in simple, neutral English. " +
		"Use only facts present in the supplied title and summary. Do not add claims, " +
		"predictions, praise, calls to action, or links. what_changed should be detailed " +
		"enough to understand the announcement. why_it_matters should explain the likely " +
		"developer relevance without claiming facts absent from the source."
