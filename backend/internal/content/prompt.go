...
Never return an empty required string, an empty required list, or an empty list item. If space is tight, shorten wording evenly across all sections while preserving every section and the minimum useful depth. A heading without developed body text is an invalid lesson.
